...
import { UserCog, Users, ShieldCheck, Activity, FileText, AlertTriangle, Save } from "lucide-react";
    ...
    setUsers((prev) =>
      prev.map((u) => {
        ...
        return {
          ...u,
          editingRoles: has
            ? u.editingRoles?.filter((r) => r !== role)
            : [...(u.editingRoles ?? []), role],
        };
      })
    );
  ...
  return (
    <div className="p-8 space-y-8">
      <div>
        ...
        <p className="text-slate-400">Manage users, roles, and system activity</p>
      </div>
        ...
        {(["users", "audit"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-all ${
              tab === t ? "bg-violet-500/20 text-violet-300 border border-violet-500/30"
            ...
            <Table>
              ...
              <TableBody>
                {users.map((u) => {
                  ...
                  return (
                    <TableRow key={u.id} className={`${u.id === user?.id ? "bg-violet-500/5" : ""}`}>
                      <TableCell>
                ...
                <TableBody>
                  {auditLogs.map((log: Record<string, unknown>) => (
                    <TableRow key={String(log.id)}>
                      ...
                      <TableCell>
                        <Badge className="text-xs">{String(log.action)}</Badge>
                      </TableCell>
                      <TableCell className="text-xs text-slate-300">{String(log.target ?? "—")}</TableCell>
                      <TableCell className="text-xs text-slate-500 font-mono max-w-[200px] truncate">
