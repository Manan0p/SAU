...
import { Heart, Eye, EyeOff, ArrowRight, Shield, Calendar, FileText, UserPlus, LogIn } from "lucide-react";
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ name: signupName, email: signupEmail, password: signupPassword, college_id: signupCollegeId }),
      });
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex gap-1 p-1 rounded-xl bg-white/5 border border-white/10 mb-8">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium rounded-lg transition-all ${tab === "login" ? "bg-violet-600 text-white shadow-lg shadow-violet-500/25" : "text-slate-400 hover:text-white"}`}
            >
              <LogIn className="w-4 h-4" /> Sign In
            </button>
            <button
              onClick={() => setTab("signup")}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium rounded-lg transition-all ${tab === "signup" ? "bg-violet-600 text-white shadow-lg shadow-violet-500/25" : "text-slate-400 hover:text-white"}`}
            >
          ...
          {tab === "login" ? (
            <>
              ...
              <p className="text-slate-400 mb-6">Sign in to your student health portal.</p>

              <div className="mb-5 p-3 rounded-xl bg-violet-500/10 border border-violet-500/20 text-xs text-violet-300">
              ...
              <form onSubmit={handleLogin} className="space-y-4" suppressHydrationWarning>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input id="password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" required className="pr-11" />
                ...
                <Button id="login-submit" type="submit" size="lg" className="w-full" disabled={isLoading}>
                  {isLoading ? <span className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" />Signing in…</span> : <span className="flex items-center gap-2">Sign in <ArrowRight className="w-4 h-4" /></span>}
                </Button>
              </form>
            </>
          ) : (
            <>
              ...
              <form onSubmit={handleSignup} className="space-y-4" suppressHydrationWarning>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="signup-email" type="email" placeholder="you@sau.edu.in" value={signupEmail} onChange={(e) => setSignupEmail(e.target.value)} required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="signup-college-id">College ID <span className="text-slate-500">(optional)</span></Label>
                  <Input id="signup-college-id" type="text" placeholder="SAU2024001" value={signupCollegeId} onChange={(e) => setSignupCollegeId(e.target.value)} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="signup-password">Password</Label>
                  <div className="relative">
                    <Input id="signup-password" type={showSignupPassword ? "text" : "password"} placeholder="Choose a strong password" value={signupPassword} onChange={(e) => setSignupPassword(e.target.value)} required className="pr-11" />
                ...
                <Button id="signup-submit" type="submit" size="lg" className="w-full bg-violet-600 hover:bg-violet-700" disabled={isSigningUp}>
                  {isSigningUp ? <span className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" />Creating account…</span> : <span className="flex items-center gap-2"><UserPlus className="w-4 h-4" /> Create Account</span>}
                </Button>
              </form>
            </>
          )}
          ...
          <p className="mt-4 text-center text-xs text-slate-700">SAU Campus Healthcare Portal · Secured by Supabase</p>
        </div>
      </div>
    </div>
  );
